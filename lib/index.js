...
exports.compose = function(manifest /*, [options], [callback] */ ) {
    ...
    async.waterfall([
        ...
        function(app, cb) {
            /*
             * Parse routes from manifest application.routes entries.
             */
            cb(null, internals.parseRoutes(manifest, app))
        }
    ], function(error, app) {
        if (error) {
...
internals.parseRoutes = function(manifest, app) {
    let routes = manifest.application.routes;
    /*
     * Parse routes from manifest.application.routes.
     */
    if (routes && routes.length) {
        routes.forEach((route) => {
            app[route.method.toLowerCase()].apply(app, [route.path, route.handlers]);
        });
    }
